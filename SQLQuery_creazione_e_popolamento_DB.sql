    ...
    alter table pizza
    add constraint fk_Pizza_Categoria foreign key(categoriaId) references Categoria(Id); -- il constraint serve per dare un nome al vincolo, se non l' avessi messo sarebbe risultato unnamed (quindi è opzionale) vedi tabella sotto PizzaIngrediente dove non c'è un add constraint;
